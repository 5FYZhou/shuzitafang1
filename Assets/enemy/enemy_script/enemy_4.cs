...
public class enemy_4 : MonoBehaviour, IHealthAccessor, enemy
{
    ...
    public int map_height, map_width;
    public int[] birth_point;//这是实际的像素点位，不是整除后的
    public int[,] map, waypoint;//要跟实际的上下颠倒一下
    public float[] attack_interval, fission_range, HP,skill_interval, original_point;//HP:{now,all}
    ...
    public void Start()
    {
        ...
        enemy_4_animation = GetComponent<Animator>();
        ...
        speed = enemy_controller_script.speed*1f;
        ...
        HP = new float[] { 1000f, 1000f };

        attack_interval = new float[] { 2f, 2f, 0 };

        skill_interval = new float[] { 3f, 0f};
        ...
        enemy_4_animation.SetBool("move", move);
        enemy_4_animation.SetBool("is_attack", is_attack);
        enemy_4_animation.SetBool("death", death);
    }
    ...
    void Update()
    {
        ...
        if (skill_interval[1] > skill_interval[0])
        {
            ...
            for (int i = 0; i < 2; i++)
            {
                if (select!=1 && self_y > 0 && map[self_y - 1, self_x] == road)
                {
                    fission(self_x, self_y - 1);
                    select = 1;
                }
                else if (select != 2 && self_x < map_width - 1 && map[self_y, self_x + 1] == road)
                {
                    fission(self_x + 1, self_y);
                    select = 2;
                }
                else if (select != 3 && self_y < map_height - 1 && map[self_y + 1, self_x] == road)
                {
                    fission(self_x, self_y + 1);
                    select = 3;
                }
                else if (select != 4 && self_x > 0 && map[self_y, self_x - 1] == road)
                {
                    fission(self_x - 1, self_y);
                    select = 4;
                }
